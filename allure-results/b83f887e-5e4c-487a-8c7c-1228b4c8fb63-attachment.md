...
- Name: demo-project.spec.js >> test-one-custom
- Location: tests\demo-project.spec.js:71:11
...
```ts
  ...
  69 | 
  70 | 
  71 | customTest("test-one-custom" , async({page,loginData})=>{
  72 |     await page.goto("https://www.saucedemo.com");
  73 | 
  74 |     const loginBtn = page.locator("#login-button");
  75 |     
  76 |     //login to website
> 77 |     await userName.fill(loginData.userName);
     |     ^ ReferenceError: userName is not defined
  78 |     await passWord.fill(loginData.passWord);
  79 |     await loginBtn.click();
  80 | }
  81 | )
```